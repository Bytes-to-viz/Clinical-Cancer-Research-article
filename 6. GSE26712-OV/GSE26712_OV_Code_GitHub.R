...
#TITLE: ANALYSIS OF THE GSE26712-OV (BONOME) DATASET
...
#1.Download the "GSE26712" series. 
library("GEOquery")
?getGEO()
gse <- getGEO("GSE26712")
...
dim(GEOexpression) #195 samples x 22.283 genes.
colnames(GEOexpression)
GEOexpression[1:5,] #The row names in our expression data are specific IDs not gene names.
featureNames(gse)[1:10]

#5. Change the names of the genes IDs to the gene names.
annotation(gse) #This is the platform used. GPL96.
...
tail(rownames(GEOexpression3), 50)

#6. Download phenotypic data.
summary(pData(gse))
phenotype <- pData(gse) #N=185 ovarian cancer samples + 10 normal samples.
...
} #WARNING: Quite slow. 30 mins aprox.
rownames(selExpr) <- selSymb
dim(selExpr)  #195 samples x 22.283-->21.156 genes.

#2. Check how many genes are duplicated. 
Duplicated <- selExpr[duplicated(rownames(selExpr)),] 
dim(Duplicated) #7.857 duplicated genes.
...
selExpr3 <- aggregate(.~geneID, selExpr2, FUN=mean) #21.156->13.299 genes. #WARNING: Quite slow.
...
##Processing:	The CEL files generated were later normalized using the RMA method.
##Values: RMA normalized signal value.
...
#2. Convert gene expression values into Z-scores ((x - mean(Xcolumn)) / sd(Xcolumn)).
...
A <- selExpr5[which(rownames(selExpr5) %in% ALTEJlist), ] #34/36 genes.

#2. Select only the TGFB and ALTEJ genes. 
genes <- as.data.frame(t(genesT))
genes <- genes[which(rownames(genes) %in% TGFBlist | rownames(genes) %in% ALTEJlist), ] #83 genes.
...
dim(GEOALL) #195 samples x 47 variables.
...
GEOALL <- merge(GEOALL, ssgsea_original[,c("ID", "balt_original", "Tertile_original", "tertile_original")], 
                by.x = "ID", by.y = "ID",
                all.x=FALSE, all.y=FALSE)
dim(GEOALL) #195 samples x 50 variables.
...
GEOALL$OS.time <- GEOALL$`survival years:ch1`
table(GEOALL$OS.time)
GEOALL$OS.months <- as.numeric(as.character(GEOALL$OS.time))
GEOALL$OS.months <- GEOALL$OS.months * 12
table(GEOALL$`status:ch1`)
GEOALL$OS <- ifelse(GEOALL$`status:ch1`=="AWD (alive with disease)", 0,
                   ifelse(GEOALL$`status:ch1`=="DOD (dead of disease)", 1, 
                   ifelse(GEOALL$`status:ch1`=="NED (no evidence of disease)", 0, NA)))

#4. Check stage. 
table(GEOALL$`tissue:ch1`) #The 185 patients had late-stage (III-IV) high-grade ovarian cancers. 

#5. Check treatment. 
##The source paper specifies that patients were treated with adjuvant platinum‐based chemotherapy.
...
#7. Exclude the normal samples.
table(GEOALL$`tissue:ch1`)
GEOALL <- GEOALL[-which(GEOALL$`tissue:ch1`=="Normal ovarian surface epithelium"),] #195->185 patients.

#8. Export the "GEOALL" dataset. 
write_xlsx(GEOALL, "Output/balt_scores_GSE26712OV.xlsx")
...
GEOALL <- read_excel("Output/balt_scores_GSE26712OV.xlsx")
...
#1. Plot and compare the overall survival curves between the weighted Balt score top and bottom tertiles.
X <- GEOALL
...
p<- ggsurvplot(my.fit, data=X, pval = TRUE, conf.int = TRUE, break.time.by = 12,
               ...
               #surv.median.line = "hv",
               risk.table = TRUE) 
p$plot <- p$plot + labs(title="GSE26712-OV", subtitle = "All (n=185)") + 
...
p<-ggsurvplot(my.fit, data=X, pval = TRUE, size = 1.5, censor.size=7, conf.int = FALSE, break.time.by = 12,
              ...
              risk.table = TRUE)
p$plot <- p$plot + labs(title="GSE26712-HNSC", subtitle = "All (n=185)") + 
  theme(plot.title=element_text(hjust = 0.5), plot.subtitle=element_text(hjust = 0.5))
p1<-p;p1 #PDF 8x7

#2. Calculate the overall survival hazard ratio between the weighted Balt score top and bottom tertiles.
my.surv.object <- Surv(time=X$OS.months, event=X$OS)
cox<-coxph(my.surv.object ~  X$Tertile_weighted)
summary(cox)

#3. Plot survival curves separating patients also by their surgery outcome.
X$newgroup <- ifelse(X$tertile_weighted=="Low" & X$`surgery outcome:ch1`=="Optimal", "low ßalt - Optimally debulked",
                  ifelse(X$tertile_weighted=="Low" & X$`surgery outcome:ch1`=="Suboptimal", "low ßalt - Suboptimally debulked",
                  ifelse(X$tertile_weighted=="High" & X$`surgery outcome:ch1`=="Optimal", "high ßalt - Optimally debulked",
                  ifelse(X$tertile_weighted=="High" & X$`surgery outcome:ch1`=="Suboptimal", "high ßalt - Suboptimally debulked",
                  NA))))
my.surv.object <- Surv(time=X$OS.months, event=X$OS)
my.fit<-survfit(my.surv.object~X$newgroup)
my.fit
p<-ggsurvplot(my.fit, data=X, pval = TRUE, conf.int = FALSE, break.time.by = 12,
              xlab="Time (months)", ylab="Surviving Fraction", xlim=c(0, 60),
              legend.labs=c("high βalt - Optimal", "high βalt - Suboptimal", "low βalt - Optimal", "low βalt - Suboptimal"), 
              legend.title=" ", 
              font.main=15, palette=c("steelblue4", "steelblue3", "indianred1", "coral"),
              risk.table = TRUE)
p$plot <- p$plot + labs(title="GSE26712-OV", subtitle = "All (n=185)") + 
  theme(plot.title=element_text(hjust = 0.5), plot.subtitle=element_text(hjust = 0.5))
p1<-p;p1 #PDF 8x7
...
#1. Plot and compare the overall survival curves between the original Balt score top and bottom tertiles.
X <- GEOALL
...
#2. Calculate the overall survival hazard ratio between the original Balt score top and bottom tertiles.
my.surv.object <- Surv(time=X$OS.months, event=X$OS)
...
##Not possible because no additional stage (all III-IV) and age info available.
